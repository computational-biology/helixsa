C
C Released version in July 13 2022, version 2.1.18
C
C
...
c	  write(*,*) "in main", val1(i)
     ...
     1        'NoHlx'.or.valp.eq.'NoHLX'.or.valp.eq.'NOhlx') then
	       nohlx = 1     ! temporary bhatta June 2021
...
160   format('#====='/'# Generated by BPFIND release July 2022, version
     1 2.1.18'/'#=====')
...
c       write(*,*) 'NdbHlx',(ndoub(jl,1),ndoub(jl,2),jl=1,NdbHlx)
     ...
     1  base1(j),base2(j)
           ...
           if(strseq(ires1(j)).ne.'n'.and.strseq(ires2(j)).ne.'n')then
             if(strseq(ires1(j)).ne.'t') strseq(ires1(j))='W'
             if(strseq(ires2(j)).ne.'t') strseq(ires2(j))='W'
             if(strseq(ires1(j)).eq.'t') strseq(ires1(j))='T'
             if(strseq(ires2(j)).eq.'t') strseq(ires2(j))='T'
           endif
           if(strseq(ires1(j)).eq.'n') strseq(ires1(j))='N'
           if(strseq(ires2(k)).eq.'n') strseq(ires2(j))='N'
           if(strseq(ires1(j)).eq.'t') strseq(ires1(j))='T'
           if(strseq(ires2(k)).eq.'t') strseq(ires2(j))='T'
         enddo
	 if(nohlx.eq.0) then
         write(9,17) 0
	 endif
...
C May 9, 2018
	   ...
	   if(strseq(ires1(j)).eq.'n') strseq(ires1(j))='N'
	   if(strseq(ires2(j)).eq.'n') strseq(ires2(j))='N'
	   if(strseq(ires1(j)).eq.'t') strseq(ires1(j))='T'
	   if(strseq(ires2(j)).eq.'t') strseq(ires2(j))='T'
         enddo

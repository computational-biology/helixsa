...
void scan_multifasta(struct multi_fasta* mfasta, const char* fasta_file)
{
      ...
      while(fgets(line, line_size, fp) != NULL) {
	    if(line[0] == '>'){
		  ...
		  fasta = mfasta->fasta + mfasta->size;
		  mfasta->size = mfasta->size + 1;

		  char sep[] = "\t :>\n";
		  char *token = strtok(line, sep);
//		  token = strtok(NULL, sep);
		  strcpy(fasta->accn, token);

		  token = strtok(NULL, sep);
		  strcpy(fasta->chain, token);
		  fasta->memsize = 1024;
		  fasta->seq = (char*) malloc ( fasta->memsize * sizeof(char) );
		  if ( fasta->seq==NULL ) {
	    ...
	    if(fasta->size > fasta->memsize - line_size -3){ // always have enough space to fit the next line.
		  ...
		  if ( fasta->seq==NULL ) {
			fprintf ( stderr, "\ndynamic memory re-allocation failed in function %s()\n" , __func__);
			exit (EXIT_FAILURE);
		  }
	    }
      }
...
void multifasta_free(struct multi_fasta* mfasta){

      for(int i=0; i<mfasta->size; ++i){
	    free ( mfasta->fasta[i].seq );
	    mfasta->fasta[i].seq = NULL;
      }
}

void scanfasta(struct fasta* fasta, int maxsize, const char* fasta_file)
{
      ...
      if(line[0] != '>'){    /* Exception Handling */ 
	    fprintf(stderr, "Error in function %s()... no comment line found at the biginning.\n", __func__);
	    exit(EXIT_FAILURE);
      }
...
void fasta_free(struct fasta* fasta){
...
static struct atom parse_cif_line_to_atom(char* line, struct cif_attrloc* attrloc){
      ...
      if(atom.loc[2]=='\'') atom.loc[2] = '*';
      if(atom.loc[3]=='\'') atom.loc[3] = '*';
